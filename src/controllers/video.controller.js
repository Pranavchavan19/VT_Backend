...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    if (query) {
        pipeline.push({
            $search: {
                index: "search-videos", // Ensure this index is created in MongoDB Atlas
                text: {
                    query: query,
                    path: ["title", "description"], // Search in title and description only
                },
            },
        });
    }
    ...
    // Handle sorting if sortBy and sortType are provided
    if (sortBy && sortType) {
        const sortOptions = { [sortBy]: sortType === "asc" ? 1 : -1 };
        pipeline.push({ $sort: sortOptions });
    } else {
        // Default to sorting by creation date if not provided
        pipeline.push({ $sort: { createdAt: -1 } });
    }
